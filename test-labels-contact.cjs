const axios = require('axios');

const headers = {
  'Content-Type': 'application/json',
  'SecretKey': '9ef1ce45-8fc8-410d-b600-fff2aea3cca7L',
  'PublicToken': '01957166-18ca-486b-aacc-df1d95690f50',
  'DeviceToken': '975e48aa-74db-464d-97a3-0ef7a523764d',
  'Authorization': 'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..47hESUk1HdzjHaoFn8mO3AhsYKiQywM6RimO0ArAuqA'
};

async function run() {
  try {
    const res = await axios.post('http://gateway.apibrasil.io/api/v2/whatsapp/getAllContacts', {}, { headers });
    const contacts = res.data.response.contacts || res.data.response || res.data;
    const sample = contacts.find(c => (c.labels && c.labels.length > 0) || (c.data && c.data.labels && c.data.labels.length > 0));
    console.log("SAMPLE CONTACT WITH LABELS:", JSON.stringify(sample, null, 2));
  } catch (e) {
    console.log("CONTACT ERR:", e.response ? e.response.data : e.message);
  }
}
run();
